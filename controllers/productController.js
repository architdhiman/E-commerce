...
export const createProductController = async (req, res) => {
  try {
    ...
    return res
      .status(404)
      .send({ message: "Error creating product", error, success: false });
  }
};
...
export const getProductController = async (req, res) => {
  try {
    ...
    return res
      .status(404)
      .send({ message: "error getting product", error, success: false });
  }
};
...
export const productFiltersController = async(req, res) => {
      try {
        ...
        let args = {}
        if (checked.lengh > 0) args.category = checked
        ...
        console.log(error);
        res.status(404).send({ message: "error while filtering product", error, success: false });
      }
}
...
export const deleteProductController = async (req, res) => {
  try {
    ...
    return res
      .status(404)
      .send({ message: "error in deleting product", error, success: false });
  }
};
...
try {
  ...
  res.status(200).send({
    message: "Total Document Counted Success",
    success:true,
    total
  })
} catch (error) {
...
export const productListController = async(req,res) =>{
  try {
    ...
    res.status(400).send({
      message: "Error in Produclist Controller",
      success: false,
      error
  })
}
